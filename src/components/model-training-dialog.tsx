  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Treinar um Novo Modelo</DialogTitle>
          <DialogDescription>
            Configure os parâmetros para seu modelo de série temporal usando o arquivo: {fileId}.
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="lstm" className="pt-4">
            ...
            <DialogFooter>
                <Button onClick={() => handleTrain('lstm')} disabled={isTraining}>
                    <BrainCircuit className="mr-2 h-4 w-4" /> {isTraining ? 'Treinando...' : 'Treinar Modelo LSTM'}
                </Button>
            </DialogFooter>
          </TabsContent>
            ...
            <div className="grid gap-4 py-4">
              <div className="flex items-center justify-between rounded-lg border p-3 shadow-sm">
                <div className="space-y-0.5">
                    <Label>Sazonalidade Trimestral</Label>
                </div>
              ...
              <div className="flex items-center justify-between rounded-lg border p-3 shadow-sm">
                <div className="space-y-0.5">
                    <Label>Sazonalidade Anual</Label>
                </div>
             ...
             <DialogFooter>
                <Button onClick={() => handleTrain('prophet')} disabled={isTraining}>
                    <BrainCircuit className="mr-2 h-4 w-4" /> {isTraining ? 'Treinando...' : 'Treinar Modelo Prophet'}
                </Button>
            </DialogFooter>
          </TabsContent>
        </Tabs>
